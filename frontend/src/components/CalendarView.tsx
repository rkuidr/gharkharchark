...
import { formatIndianRupee, formatDate, getDaysInMonth, getFirstDayOfMonth } from '../utils/formatters';
...
export function CalendarView({ expenses, onEditExpense }: CalendarViewProps) {
  ...
  const selectedTotal = useMemo(() => sumExpenses(selectedExpenses), [selectedExpenses]);
    ...
    <div className="pb-24">
      ...
      <div className="bg-gk-primary px-5 pt-12 pb-5">
        ...
        <div className="flex items-center justify-between">
          <button onClick={prevMonth} className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white">
            <ChevronLeft size={20} />
          </button>
          <h2 className="text-white font-bold text-base">{monthLabel}</h2>
          <button onClick={nextMonth} className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white">
      ...
      <div className="px-4 -mt-2">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card overflow-hidden mt-4">
          ...
          <div className="grid grid-cols-7 border-b border-gray-100 dark:border-gray-700">
            {weekDays.map(d => (
              <div key={d} className="py-2 text-center text-[11px] font-bold text-gk-text-secondary">{d}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7">
            {Array.from({ length: firstDay }).map((_, i) => (
              <div key={`empty-${i}`} className="h-14 border-b border-r border-gray-50 dark:border-gray-700/50" />
            ))}
              ...
              return (
                <button
                  ...
                  className={`h-14 flex flex-col items-center justify-center border-b border-r border-gray-50 dark:border-gray-700/50 transition-colors ${
                    isSelected ? 'bg-green-50 dark:bg-green-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'
                  }`}
                >
                  <span className={`text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full ${
                    isToday ? 'bg-gk-accent text-white' :
                    isSelected ? 'text-gk-accent' :
        ...
        {selectedDate && (
          <div className="mt-4 bg-white dark:bg-gray-800 rounded-2xl shadow-card overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <div>
                <h3 className="text-gk-text dark:text-white font-bold text-sm">{formatDate(selectedDate)}</h3>
                <p className="text-gk-text-secondary text-xs">{selectedExpenses.length} expense{selectedExpenses.length !== 1 ? 's' : ''}</p>
              </div>
              <p className="text-gk-accent font-bold text-base">{formatIndianRupee(selectedTotal)}</p>
            </div>

            {selectedExpenses.length === 0 ? (
              <div className="py-8 text-center">
                <p className="text-3xl mb-2">📭</p>
                <p className="text-gk-text-secondary text-sm">No expenses on this day</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={expense.id} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gk-text-secondary text-xs">{expense.paymentMode}{expense.notes ? ` · ${expense.notes}` : ''}</p>
                      </div>
