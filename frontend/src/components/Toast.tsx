...
function ToastItem({ toast, onRemove }: { toast: ToastItem; onRemove: (id: string) => void }) {
  ...
  return (
    <div className={`toast-enter flex items-center gap-3 px-4 py-3 rounded-xl shadow-card-hover ${bgColors[toast.type]}`}>
      {icons[toast.type]}
      <span className="text-sm font-medium text-gk-text flex-1">{toast.message}</span>
      <button onClick={() => onRemove(toast.id)} className="text-gk-text-secondary hover:text-gk-text">
        <X size={16} />
      </button>
    </div>
  );
