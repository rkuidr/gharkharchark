...
export function PINLock({ correctPin, onUnlock }: PINLockProps) {
  ...
  return (
    <div className="fixed inset-0 bg-gk-primary flex flex-col items-center justify-center z-[9999] px-8">
      <img src="/assets/generated/gharkharcha-logo.dim_128x128.png" alt="GharKharcha" className="w-16 h-16 rounded-2xl mb-4" />
      <h1 className="text-white text-2xl font-bold mb-1">GharKharcha</h1>
        ...
        {[0, 1, 2, 3].map(i => (
          <div
            ...
            className={`w-4 h-4 rounded-full border-2 transition-all duration-200 ${
              i < pin.length
                ? error ? 'bg-gk-danger border-gk-danger' : 'bg-gk-accent border-gk-accent'
                : 'border-white/40'
            }`}
          />
        ))}
      ...
      {error && <p className="text-gk-danger text-sm font-semibold mb-4">Incorrect PIN. Try again.</p>}
