...
import { Filter, Download, Share2, Printer, X } from 'lucide-react';
...
import { getCategoryConfig, CATEGORIES, PAYMENT_MODES } from '../utils/categoryConfig';
import { triggerCSVDownload, formatTextSummary, copyToClipboard } from '../utils/exportUtils';
...
export function Reports({ expenses }: ReportsProps) {
  ...
  const handleShare = async () => {
    ...
    showToast(ok ? 'Report copied to clipboard!' : 'Failed to copy', ok ? 'success' : 'error');
  };
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        {showFilters && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card space-y-4">
            <h3 className="text-gk-text dark:text-white font-bold text-sm">Filter Expenses</h3>

            {/* Date Range */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-gk-text-secondary mb-1">From</label>
                <input
                  type="date"
                  value={fromDate}
              ...
              <div>
                <label className="block text-xs font-semibold text-gk-text-secondary mb-1">To</label>
                <input
                  type="date"
                  value={toDate}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-semibold text-gk-text-secondary">Categories</label>
                {selectedCategories.length > 0 && (
                  <button onClick={() => setSelectedCategories([])} className="text-xs text-gk-accent font-semibold">Clear</button>
                )}
              </div>
            ...
            {/* Payment Mode */}
            <div>
              <label className="block text-xs font-semibold text-gk-text-secondary mb-2">Payment Mode</label>
              <div className="flex flex-wrap gap-2">
        ...
        <div className="bg-gk-primary rounded-2xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-white text-2xl font-bold">{formatIndianRupee(total)}</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            ...
            <span className="text-xs font-semibold text-gk-text-secondary">PDF/Print</span>
          </button>
          <button
            onClick={handleCSV}
            className="flex flex-col items-center gap-2 bg-white dark:bg-gray-800 rounded-2xl p-3 shadow-card"
          >
            ...
            <span className="text-xs font-semibold text-gk-text-secondary">CSV Export</span>
          </button>
          <button
            onClick={handleShare}
            className="flex flex-col items-center gap-2 bg-white dark:bg-gray-800 rounded-2xl p-3 shadow-card"
          >
            ...
            <span className="text-xs font-semibold text-gk-text-secondary">Share</span>
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card overflow-hidden">
          ...
          {filtered.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-gk-text dark:text-white font-semibold">No expenses found</p>
              <p className="text-gk-text-secondary text-sm mt-1">Try adjusting your filters</p>
            </div>
          ) : (
                ...
                return (
                  <div key={expense.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-gk-text dark:text-white text-sm font-semibold">{expense.category}</p>
                      <p className="text-gk-text-secondary text-xs">{formatDate(expense.date)} · {expense.paymentMode}</p>
                      {expense.notes && <p className="text-gk-text-secondary text-xs truncate">{expense.notes}</p>}
                    </div>
