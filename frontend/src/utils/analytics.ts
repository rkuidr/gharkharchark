...
export function groupByDay(expenses: Expense[], year: number, month: number): Record<string, number> {
  const result: Record<string, number> = {};
  const prefix = `${year}-${String(month).padStart(2, '0')}`;
  for (const e of expenses) {
...
export function groupByMonth(expenses: Expense[], year: number): Record<number, number> {
...
export function getTopCategory(expenses: Expense[]): { name: string; amount: number } | null {
...
export function getPieChartData(expenses: Expense[]): Array<{ name: string; value: number }> {
  const grouped = groupByCategory(expenses);
...
export function getDailyBarData(expenses: Expense[], year: number, month: number): Array<{ day: string; amount: number }> {
...
export function getMonthlyBarData(expenses: Expense[], year: number): Array<{ month: string; amount: number }> {
  const grouped = groupByMonth(expenses, year);
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  return months.map((m, i) => ({ month: m, amount: Math.round(grouped[i + 1] || 0) }));
}
