...
function MonthlyAnalysis({
  expenses,
  settings,
}: { expenses: Expense[]; settings: Settings }) {
  const now = new Date();
  ...
  const prevMonthExpenses = useMemo(
    () => filterByMonth(expenses, prevMonthDate.year, prevMonthDate.month),
    [expenses, prevMonthDate.year, prevMonthDate.month],
  );
  ...
  const budgetPct =
    settings.monthlyBudget > 0
      ? Math.min((total / settings.monthlyBudget) * 100, 100)
      : 0;
  ...
  const dailyData = useMemo(
    () => getDailyBarData(monthExpenses, year, month),
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="flex items-center justify-between bg-white dark:bg-gray-800 rounded-2xl px-4 py-3 shadow-card">
        ...
        <h2 className="text-gk-text dark:text-white font-bold text-base">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-gk-text dark:text-white text-3xl font-bold">
          ...
          <div
            className={`px-3 py-1 rounded-full text-xs font-bold ${
              total <= settings.monthlyBudget
                ? "bg-green-100 text-gk-accent"
                : "bg-red-100 text-gk-danger"
            }`}
          >
            {total <= settings.monthlyBudget
              ? "✅ Under Budget"
        ...
        {prevTotal > 0 && (
          <div
            className={`flex items-center gap-1 mt-3 text-xs font-semibold ${pctChange > 0 ? "text-gk-danger" : "text-gk-accent"}`}
          >
            <span>{pctChange > 0 ? "↑" : "↓"}</span>
            <span>
              {Math.abs(Math.round(pctChange))}% vs last month (
              {formatIndianRupee(prevTotal)})
            </span>
          </div>
        )}
      </div>
      ...
      {topCat && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gk-text dark:text-white font-bold">
              {topCat.name}
            </p>
            <p className="text-gk-accent text-sm font-bold">
              {formatIndianRupee(topCat.amount)}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              <Legend
                formatter={(v) => (
                  <span className="text-xs text-gk-text-secondary">{v}</span>
                )}
                iconSize={8}
              />
            </PieChart>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
          ...
          <ResponsiveContainer width="100%" height={180}>
            ...
            >
              ...
              />
              <Tooltip
                formatter={(v: number) => formatIndianRupee(v)}
...
function YearlyAnalysis({
  ...
  const yearExpenses = useMemo(
    () => filterByYear(expenses, year),
    [expenses, year],
  );
  const monthlyData = useMemo(
    () => getMonthlyBarData(yearExpenses, year),
    [yearExpenses, year],
  );
    ...
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
          ...
          <p className="text-gk-text dark:text-white text-lg font-bold mt-1">
            {formatIndianRupee(total)}
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
          ...
          <p className="text-gk-text dark:text-white text-lg font-bold mt-1">
            {formatIndianRupee(avgMonthly)}
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
          <p className="text-gk-text-secondary text-xs">Highest Month</p>
          <p className="text-gk-accent text-lg font-bold mt-1">
            {highestMonth?.month || "-"}
          </p>
          <p className="text-gk-text-secondary text-xs">
            {highestMonth ? formatIndianRupee(highestMonth.amount) : ""}
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
          <p className="text-gk-text-secondary text-xs">Est. Savings</p>
          <p
            className={`text-lg font-bold mt-1 ${savings >= 0 ? "text-gk-accent" : "text-gk-danger"}`}
          >
              ...
              <Bar dataKey="amount" radius={[4, 4, 0, 0]}>
                {monthlyData.map((entry) => (
                  <Cell
                    ...
                    fill={
                      entry.month === highestMonth?.month
                        ? "#EF4444"
                        : "#22C55E"
                    }
                  />
